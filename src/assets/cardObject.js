...
class Card {
    ...
    getInfo() {
        ...
        if (bids.length > 0) {
            info.highestBid = parseFloat(bids[bids.length - 1].getElementsByClassName('cardprice')[0].innerHTML)
        } else {
            info.highestBid = 0
        }
        let asks = [...document.getElementsByClassName('cardauctionunmatchedrow-ask')]
        if (asks.length > 0) {
            info.lowestAsk = parseFloat(asks[0].getElementsByClassName('cardprice')[0].innerHTML)
        }
