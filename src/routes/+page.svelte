<script lang="ts">
	...
	let loadProjects = async (): Promise<Project[]> => {
		...
		projectJson.forEach((p) => {
			...
			if (isMonth(p.month)) {
				projects.push(new Project(p.title, p.tags, p.year, p.month, curSide, swapNeeded));
				swapNeeded = false;
			}
		});
...
<div class="lg:flex justify-space-between min-h-full">
	<div class="sidebar p-10 lg:w-1/3 lg:border-r-4 flex flex-col justify-start items-center">
		<img src="/portraitSquareBlurred.jpg" alt="Me" class="rounded-lg width-max" />
		<H1>Ac Hýbl</H1>
		<a href="https://github.com/MatousAc">Github</a>
	</div>
	<div class="lg:px-5 lg:w-2/3 py-10">
		{#await loadProjects()}
			<p>...waiting</p>
		{:then projects}
			<H2>Portfolio</H2>
